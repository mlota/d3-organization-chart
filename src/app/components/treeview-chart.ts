...
export interface ChartProps {
  ...
  data: any; // Check, might want this as typed
  depth: number;
  duration: number;
  strokeWidth: number;
  dropShadowId: null;
...
export class Chart {
  // private getChartState: () => ChartProps;
  // private _attrs: ChartProps;
  private _attrs: any;
  ...
  public setData(data: any) {
    this._attrs.data = data;
  }

  public setContainer(container: string) {
    this._attrs.container = container;
  }

  initializeEnterExitUpdatePattern() {
    d3.selection.prototype.patternify = function(params: any) {
      var container = this;
  ...
  setZoomFactor(zoomLevel) {
    ...
    const attrs: any = this._attrs;
    const calc: any = attrs.calc;
  ...
  render() {
    ...
    calc.nodeMaxWidth = d3Array.max(attrs.data, ({ width }) => width);
    calc.nodeMaxHeight = d3Array.max(attrs.data, ({ height }) => height);
    ...
    behaviors.zoom = d3Zoom.zoom().on('zoom', (d: any) => this.zoomed());

    //****************** ROOT node work ************************

    // Convert flat data to hierarchical
    attrs.root = d3Hierarchy
      .stratify()
      .id(({ nodeId }) => nodeId)
    ...
    attrs.root.children.forEach((d: any) => this.expandSomeNodes(d));
  ...
  removeNode(nodeId) {
    ...
    if (node) {
      ...
      attrs.data = attrs.data.filter(
        (d: any) => !nodeChildrenIds.includes(d.nodeId)
      );
  ...
  update({ x0, y0, x, y }) {
    ...
    const nodes = treeData.descendants().map((d: any) => {
    ...
    nodes.forEach((d: any) => (d.y = d.depth * attrs.depth));
    ...
    const patternImages = patterns
      ...
      .attr('xlink:href', ({ data }) => data.nodeImage.url)
      .attr(
        'viewbox',
    ...
    linkUpdate
      ...
      .attr(
        'stroke-width',
        ({ data }: { data: any }) => data.connectorLineWidth || 2
      )
      .attr('stroke', ({ data }) => {
    ...
    const nodeEnter = nodesSelection
      ...
      .attr('pointer-events', 'all')
      .attr('class', 'node')
      .attr('transform', (d: any) => `translate(${x0},${y0})`)
      .attr('cursor', 'pointer')
      .on('click', ({ data }) => {
    ...
    nodeEnter
      ...
      .attr('width', ({ data }) => data.nodeIcon.size)
      .attr('height', ({ data }) => data.nodeIcon.size)
      .attr('xlink:href', ({ data }) => data.nodeIcon.icon)
      .attr('x', ({ width }) => -width / 2 + 5)
      .attr('y', ({ height, data }) => height / 2 - data.nodeIcon.size - 5);
    ...
    nodeEnter
      ...
      .text('test')
      .attr('x', ({ width }) => -width / 2 + 7)
      .attr('y', ({ height, data }) => height / 2 - data.nodeIcon.size - 5)
      .text(({ data }) => `${data.totalSubordinates} Subordinates`)
    ...
    nodeEnter
      ...
      .text('test')
      .attr('x', ({ width, data }) => -width / 2 + 10 + data.nodeIcon.size)
      .attr('y', ({ height }) => height / 2 - 10)
      .text(({ data }) => `${data.directSubordinates} Direct `)
      .attr('fill', attrs.nodeTextFill)
      .attr('font-weight', 'bold')
      .attr('display', attrs.hideSubordinates ? 'none' : 'initial');
    ...
    const nodeButtonGroups = nodeEnter
      ...
      .on('mousedown', (d: any) => this.onButtonClick(d));
    ...
    const nodeExitTransition = nodesSelection
      ...
      .transition()
      .duration(attrs.duration)
      .attr('transform', (d: any) => `translate(${x},${y})`)
      .on('end', function() {
   ...
   */
  setExpanded(id: any, expandedFlag: boolean) {
    // const attrs: any = this.getChartState();
    const attrs = this._attrs;
    ...
    attrs.root.children.forEach((d: any) => this.expand(d));
  ...
  expandSomeNodes(d: any) {
    ...
    if (d._children) {
      d._children.forEach((ch: any) => this.expandSomeNodes(ch));
    }

    // Recursivelly do the same for expanded nodes
    if (d.children) {
      d.children.forEach((ch: any) => this.expandSomeNodes(ch));
    }
  }
  ...
  updateNodesState() {
    ...
    attrs.root.children.forEach((ch: any) => this.expandSomeNodes(ch));
  ...
  collapse(d: any) {
    if (d.children) {
      d._children = d.children;
      d._children.forEach((ch: any) => this.collapse(ch));
      d.children = null;
    }
  }
  ...
  expand(d: any) {
    if (d._children) {
      d.children = d._children;
      d.children.forEach((ch: any) => this.expand(ch));
      d._children = null;
    }
  }
